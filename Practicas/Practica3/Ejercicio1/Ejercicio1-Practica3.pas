...
var
	...
	nro,pos:integer;
begin
	writeln('Ingrese nro del empleado a eliminar');
	readln(nro);
	reset(a);
	leerArchivo(a,e);
	while((e.nro <> valorAlto)and (e.nro <> nro))do
		leerArchivo(a,e);
	if(e.nro = nro) then begin
		pos := filePos(a)-1; // Guardo ubicacion del registro a eliminar
		Seek(a,filesize(a)-1); // Me ubico en el ultimo registro
		leerArchivo(a,e); // Leo el ultimo registro
		Seek(a,pos); // Me ubico en el registro a eliminar
		write(a,e); // Elimino el registro sobreescribiendo
		Seek(a,fileSize(a)-1); // Me ubico en el ultimo registro
		Truncate(a); // Trunco el archivo
	end
	else
		writeln('No se encontro el nro ');
	close(a);
